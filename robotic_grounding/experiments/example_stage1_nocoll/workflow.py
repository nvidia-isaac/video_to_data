...
For each of the 6 objects:
  1. Ensures *_no_collision.urdf exists (generates it from *_rigid.urdf if missing).
  2. Launches a 1000-iteration training task using that URDF with only the
     hand_keypoints_tracking_exp reward active.
  3. After training, uploads the final checkpoint as a W&B artifact so that
     launch_stage2.py can retrieve it for the second training stage.

The no_collision URDF is generated by replacing the mesh collision geometry
with a 1 cm box, making the object effectively phantom for contact purposes.
"""
...
from experiments.utils import (  # noqa: E402
    generate_no_collision_urdfs,  # noqa: F401 (re-exported for external callers)
    overrides_to_cli,
    sequence_to_object,
)
...
def get_variant_overrides(variant_name: str, config: dict) -> dict:
    ...
    Pass --no-collision on the CLI to use the collision-free URDF.
    """
...
def generate_workflow(exp_id: str, config: dict) -> str:
    ...
    # Generate no_collision URDFs locally at workflow generation time
