...
def fetch_crashed_checkpoints(outdir: Path, config: dict) -> dict[str, Path]:
    ...
    for seq_id in sequences:
        ...
        if not crashed:
            ...
            continue
        run = crashed[0]
        print(f"  Found crashed run: {run.display_name} (step={run.summary.get('_step', 'N/A')})", end=" ", flush=True)
...
def _make_task_yaml(obj: str, seq_id: str, ckpt_path: Path, exp_id: str,
                    config: dict, wandb_api_key: str) -> str:
    project = config.get("wandb_project", "v2p_hands")
...
def generate_workflow(exp_id: str, config: dict, checkpoints: dict[str, Path]) -> str:
    ...
    for seq_id in sequences:
        ...
        if seq_key not in checkpoints:
            continue
        tasks.append(_make_task_yaml(
            _sequence_to_object(seq_id), seq_id,
            checkpoints[seq_key],
            exp_id, config, wandb_api_key,
        ))
...
def main() -> None:
    ...
    parser.add_argument("--config-id", default="stage2", help="Stage 2 experiment config id from registry (default: stage2)")
    parser.add_argument("--exp-id", default=None, help="Override OSMO job name (defaults to config id)")
    parser.add_argument("--run-name-suffix", default=None, help="Override run_name_suffix from config")
    parser.add_argument("--pool", default="isaac-dev-l40s-04")
    parser.add_argument("--priority", default="NORMAL")
    parser.add_argument("--build-image", action="store_true")
    parser.add_argument("--image", default=None, help="Use specific Docker image for OSMO")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            cmd = [
                sys.executable, str(run_osmo_py),
                "--experiment-name", exp_id,
                "--workflow-yaml", str(workflow_path),
                "--pool", args.pool,
