...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Regex pattern filtering sequences (re.search over sequence_id).",
    )
    parser.add_argument(
        "--max_sequences",
        type=int,
    ...
    discovered = len(sequences)
    applied_filters: list[str] = []
    if args.sequence_id:
        sequences = [(sid, p) for sid, p in sequences if sid == args.sequence_id]
        applied_filters.append(f"--sequence_id {args.sequence_id}")
    if args.sequence_pattern:
        pat = re.compile(args.sequence_pattern)
        sequences = [(sid, p) for sid, p in sequences if pat.search(sid)]
        applied_filters.append(f"--sequence_pattern {args.sequence_pattern}")
    if args.max_sequences is not None:
        sequences = sequences[: args.max_sequences]
        applied_filters.append(f"--max_sequences {args.max_sequences}")

    if not sequences:
        print(
            f"No sequences matched {' '.join(applied_filters)} "
            f"({discovered} discovered in {args.input_dir}). "
            "Nothing evaluated; no output files written.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Existing result files are left untouched rather than truncated to a bare header.
    if not results:
        print(
            f"None of the {total} selected sequences could be read; "
