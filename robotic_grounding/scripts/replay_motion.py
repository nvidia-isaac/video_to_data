...
# Progress logging cadence, counted in simulation iterations rather than
# trajectory frames: the frame cursor advances by ``sim.dt * fps * speed``, so a
# frame-modulo test would repeat a milestone when that step is < 1 and skip it
# entirely when it is > 1.
PROGRESS_LOG_EVERY_N_STEPS = 100
...
def main() -> None:
    ...
    print("[INFO] Starting kinematic trajectory replay...")
    ...
    while simulation_app.is_running():
        ...
        frame_f += frame_step
        step_idx += 1
        if step_idx % PROGRESS_LOG_EVERY_N_STEPS == 0:
            secs = t / traj.fps if traj.fps else 0
            print(f"[INFO] Frame {t}/{traj.num_frames} ({secs:.1f}s)")

    env.close()

    print("[INFO] Kinematic trajectory replay successfully completed.")
