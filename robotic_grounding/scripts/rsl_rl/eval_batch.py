...
def main(env_cfg: ManagerBasedRLEnvCfg, agent_cfg):
    ...
    ckpts = sorted(glob.glob(os.path.join(args_cli.checkpoints_dir, "model_*.pt")), key=_itr)
    if not ckpts:
        print(f"[eval_batch] No model_*.pt found in {args_cli.checkpoints_dir}")
        return

    print(f"[eval_batch] Found {len(ckpts)} checkpoints: "
    ...
    for ckpt_path in ckpts:
        ...
        std_r = (sum((x - mean_r) ** 2 for x in ratios) / max(len(ratios) - 1, 1)) ** 0.5
        results.append((iteration, mean_r, std_r, n_full, len(data)))
        print(f"  step={iteration:6d}  ratio={mean_r:.3f}±{std_r:.3f}  "
              f"full={n_full}/{len(data)} ({100*n_full/len(data):.0f}%)",
              flush=True)

    print("\n[eval_batch] ===== Full Table =====")
    print(f"  Trajectory: {traj_len} steps, warmup: {_warmup}, episodes/ckpt: {args_cli.eval_episodes}")
