...
"""Retarget OakInk2 loaded data (ManoSharpaData with MANO+object only) to Sharpa.

Reads the oakink2_loaded Parquet (ManoSharpaData; produced upstream by
reconstruction's v2d_task_library_loader load workflow),
runs IK per frame to fill robot_* fields, and saves to oakink2_processed.

Usage:
  1. (upstream) reconstruction load workflow writes oakink2_loaded Parquet
  2. python scripts/retarget/oakink2_to_sharpa.py --save
"""
...
def _load_object_viser_handles(
    ...
    """Load OakInk2 object meshes from stored schema paths and add to viser scene.
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the OakInk2-to-Sharpa retargeting script."""
    parser = argparse.ArgumentParser(
        description="Retarget OakInk2 loaded Parquet data to Sharpa (run IK, fill robot_*)."
    )
...
def main(args: argparse.Namespace) -> None:
    """Read loaded OakInk2 Parquet, run IK per frame, save retargeted Parquet."""
