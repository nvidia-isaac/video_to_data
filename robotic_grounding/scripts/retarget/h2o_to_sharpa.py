...
DEFAULT_INPUT_DIR = HUMAN_MOTION_DATA_DIR / "h2o" / "h2o_loaded"
DEFAULT_OUTPUT_DIR = HUMAN_MOTION_DATA_DIR / "h2o" / "h2o_processed"

# H2O uses no wrist link-to-site rotation offset (same as Hot3D, OakInk2).
H2O_LINK_TO_SITE_QUAT_XYZW = None
...
def _load_object_viser_handles(
    ...
    """Load H2O object meshes from stored schema paths and add to viser scene."""
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the H2O-to-Sharpa retargeting script."""
    parser = argparse.ArgumentParser(
        description="Retarget H2O loaded Parquet data to Sharpa (run IK, fill robot_*)."
    )
...
def main(args: argparse.Namespace) -> None:
    """Read loaded H2O Parquet, run IK per frame, save retargeted Parquet."""
    ...
    link_to_site_xyzw = H2O_LINK_TO_SITE_QUAT_XYZW
