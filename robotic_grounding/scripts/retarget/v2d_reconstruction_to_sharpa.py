...
"""Retarget V2D reconstruction loaded data to Sharpa.

Reads Parquet from v2d_reconstruction_loader.py output (v2d_reconstruction_loaded),
runs IK per frame to fill robot_* fields, and saves to v2d_reconstruction_processed.

Usage:
  1. python scripts/retarget/v2d_reconstruction_loader.py --save
  2. python scripts/retarget/v2d_reconstruction_to_sharpa.py --save
"""
...
DEFAULT_INPUT_DIR  = HUMAN_MOTION_DATA_DIR / "v2d_reconstruction" / "v2d_reconstruction_loaded"
DEFAULT_OUTPUT_DIR = HUMAN_MOTION_DATA_DIR / "v2d_reconstruction" / "v2d_reconstruction_processed"

# V2D reconstruction has no wrist link-to-site rotation offset.
V2D_LINK_TO_SITE_QUAT_XYZW = None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Retarget V2D reconstruction loaded parquet to Sharpa (run IK, fill robot_*)."
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    link_to_site_xyzw = V2D_LINK_TO_SITE_QUAT_XYZW
    ...
    for sequence_id in tqdm(sequence_ids):
        ...
        for t in range(num_frames):
            right_joints      = torch.tensor(data.mano_right_joints[t],      dtype=torch.float32, device=device)
            right_joints_wxyz = torch.tensor(data.mano_right_joints_wxyz[t], dtype=torch.float32, device=device)
            left_joints       = torch.tensor(data.mano_left_joints[t],       dtype=torch.float32, device=device)
            left_joints_wxyz  = torch.tensor(data.mano_left_joints_wxyz[t],  dtype=torch.float32, device=device)
            ...
            robot_right_wrist_wxyz.append(right_results["q"][3:7][[3, 0, 1, 2]].tolist())
            ...
            robot_right_num_optimization_iterations.append(right_results["num_optimization_iterations"])
            ...
            robot_left_num_optimization_iterations.append(left_results["num_optimization_iterations"])

        if args.save:
            d = data.to_dict()
            d["right_robot_finger_joint_names"] = list(right_sharpa_kinematics.robot_finger_joint_names.values())
            d["right_robot_frame_names"]        = list(right_sharpa_kinematics.robot_frame_names.values())
            d["right_robot_frame_task_names"]   = list(right_sharpa_kinematics.frame_tasks.keys())
            d["left_robot_finger_joint_names"]  = list(left_sharpa_kinematics.robot_finger_joint_names.values())
            d["left_robot_frame_names"]         = list(left_sharpa_kinematics.robot_frame_names.values())
            d["left_robot_frame_task_names"]    = list(left_sharpa_kinematics.frame_tasks.keys())
            ...
            d["robot_right_num_optimization_iterations"] = robot_right_num_optimization_iterations
            ...
            d["robot_left_num_optimization_iterations"] = robot_left_num_optimization_iterations
