...
DEFAULT_INPUT_DIR = HUMAN_MOTION_DATA_DIR / "grab" / "grab_loaded"
DEFAULT_OUTPUT_DIR = HUMAN_MOTION_DATA_DIR / "grab" / "grab_processed"

# GRAB uses no wrist link-to-site rotation offset (same as Hot3D, OakInk2, H2O).
GRAB_LINK_TO_SITE_QUAT_XYZW = None
...
def _load_object_viser_handles(
    ...
    """Load GRAB object meshes from stored schema paths and add to viser scene."""
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the GRAB-to-Sharpa retargeting script."""
    parser = argparse.ArgumentParser(
        description="Retarget GRAB loaded Parquet data to Sharpa (run IK, fill robot_*)."
    )
...
def main(args: argparse.Namespace) -> None:
    """Read loaded GRAB Parquet, run IK per frame, save retargeted Parquet."""
    ...
    link_to_site_xyzw = GRAB_LINK_TO_SITE_QUAT_XYZW
