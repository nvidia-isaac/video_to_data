...
# Default paths: loader output -> retarget output
...
# Dex3 MANO retargeting does not need a link-to-site wrist offset
LINK_TO_SITE_XYZW = None
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Retarget TACO loaded Parquet data to Dex3 (run IK, fill robot_*)."
    )
...
def main(args: argparse.Namespace) -> None:
    """Read loaded TACO Parquet, run Dex3 IK per frame, save retargeted Parquet."""
    ...
    right_dex3_kinematics = setup_dex3_kinematics(
        side="right", frame_tasks_converged_threshold=1e-6
    )
    left_dex3_kinematics = setup_dex3_kinematics(
    ...
    for sequence_id in tqdm(sequence_ids):
        ...
        for t in range(num_frames):
            ...
            right_qpos, left_qpos, right_results, left_results = run_frame_ik(
                right_dex3_kinematics,
                left_dex3_kinematics,
                right_joints,
                right_joints_wxyz,
                left_joints,
                left_joints_wxyz,
            ...
            # Free-flyer qpos layout: [pos(3), quat_xyzw(4), fingers(7)]
            # Convert xyzw -> wxyz for ManoDex3Data schema
        ...
        if args.save:
            ...
            d["right_robot_finger_joint_names"] = list(
                right_dex3_kinematics.robot_finger_joint_names.values()
            )
            d["right_robot_frame_names"] = list(
                right_dex3_kinematics.robot_frame_names.values()
            )
            d["right_robot_frame_task_names"] = list(
                right_dex3_kinematics.frame_tasks.keys()
            )
            d["left_robot_finger_joint_names"] = list(
                left_dex3_kinematics.robot_finger_joint_names.values()
            )
            d["left_robot_frame_names"] = list(
                left_dex3_kinematics.robot_frame_names.values()
            )
            d["left_robot_frame_task_names"] = list(
                left_dex3_kinematics.frame_tasks.keys()
            )
