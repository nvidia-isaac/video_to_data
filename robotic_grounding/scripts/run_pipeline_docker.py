...
DEFAULT_STAGES = ["load", "urdf", "processed", "support", "vis", "sdf", "assess"]
...
RECOMMENDED_STAGES = {
    "taco": ["load", "urdf", "processed", "support", "vis", "sdf", "assess"],
    "hot3d": [
        "load",
        "urdf",
        "processed",
    ...
    "oakink2": ["load", "urdf", "processed", "support", "vis", "sdf", "assess"],
    "arctic": [
        "load",
        "processed",
        "support",
    ...
    "h2o": ["load", "urdf", "processed", "support", "vis", "sdf", "assess"],
    "dexycb": ["load", "urdf", "processed", "support", "vis", "sdf", "assess"],
}
...
def stage_sdf(args: argparse.Namespace, ds: str, dry: bool) -> None:
    """SDF stage: precompute per-object SDFs for the penetration check.

    The assess check then measures the true (hollow-aware) object solid instead of
    its convex hull. Driven from {ds}_processed so it builds exactly the meshes every
    sequence references. Idempotent — objects already built are skipped.
    """
    p = _c_paths(ds)
    inner = _inner(
        [
            "python",
            "scripts/build_object_sdfs.py",
            "--processed-dir",
            p["processed"],
        ]
    )
