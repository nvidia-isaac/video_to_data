...
Data filtering (the `assess` stage) is OPT-IN and tunable:
  --assess                      run quality checks (report-only CSV)
...
def _require_sequences(
    ...
    when LOAD produced an empty dataset (usually: raw data missing/incomplete).
    """
    if dry:
        return
