...
def _is_lfs_pointer(path: Path) -> bool:
    """True when ``path`` is an unresolved Git-LFS pointer stub.

    A stub is a ~130-byte text file, so it passes ``is_file()`` but fails
    inside whichever parser consumes it (``np.load`` for the npz,
    ``torch.load`` for the checkpoints). Treating stubs as absent keeps the
    failure at the actionable "populate the cache" message instead of an
    opaque decode error several frames deeper.
    """
    try:
        ...
        *,
        ...
        with torch.no_grad():
            ...
            output = self.layer(
                ...
                transl=transl_t,
                # Pose-dependent corrective vertex offsets are only available in
                # procedural mode (disabled in __init__), and SOMALayer raises
                # rather than degrading if asked for them. Joint positions and
                # rotations are unaffected — correctives perturb rest-shape
                # vertices, not forward kinematics.
