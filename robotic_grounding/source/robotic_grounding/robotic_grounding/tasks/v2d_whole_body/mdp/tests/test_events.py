...
if APP_IS_READY:
    ...
    from robotic_grounding.motion_schema.reader import load_motion_data_parquet
...
class TestEvents(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.env_cfg = G1SonicEnvCfg(scene_config_path=WHOLE_BODY_EXAMPLE_MOTION)
        ...
        # Reset lands on the window start, so the expected frame is deterministic.
        cls.env_cfg.commands.motion.always_reset_to_first_frame = True
        cls.env_cfg.events.reset_to_trajectory_frame.params["trajectory_time_index"] = (
            0,
            1,
        )
        cls.motion_file = WHOLE_BODY_EXAMPLE_MOTION
    ...
    def test_reset_to_trajectory_start(self) -> None:
        ...
        motion_data = load_motion_data_parquet(self.motion_file, device=self.env.device)

        # Extract first frame data
        expected_root_pos = motion_data.robot_root_position[0]  # (3,)
        expected_root_quat = motion_data.robot_root_wxyz[0]  # (4,)
