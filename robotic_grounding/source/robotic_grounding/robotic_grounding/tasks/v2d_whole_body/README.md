...
python scripts/rsl_rl/train.py --headless \
    ...
    --motion_file whole_body/soma/sequence_id=2026-03-06_10-24-18_snack_box_pick_and_place_01/robot_name=g1 \
...
> **Note:** evaluate with the base task (`SonicG1-ReconHand-v0` /
> `SonicG1-ReconBody-v0`), not a `-Stage{1,2,3}-v0` task — the stage tasks bake
> training-time physics deltas (e.g. stage 1 disables robot↔object collisions)
> that make eval results meaningless, while their checkpoints load in the base
> task unchanged (same observation/action spaces). Use a stage-2 or stage-3
