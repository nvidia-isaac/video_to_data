...
"""Train the reference-tracking policy with the Warp RL environment and JAX PPO."""
...
from flash_chord.training.ppo.runner import WandbLogger, metric_definitions, run_training
...
@hydra.main(version_base="1.3", config_path="../src/flash_chord/configs", config_name="train")
def main(cfg: DictConfig) -> None:
    """Compose the experiment and run PPO; all experiment settings are Hydra overrides."""
    ...
    with wp.ScopedDevice(training.device):
        ...
        learner = run_training(
            ...
            progress=ConsoleTrainingProgress(
                training.max_iterations * training.world_count * training.rollout_steps,
                output_dir,
            ),
        )
    print(f"completed {int(learner.iteration)} iterations; checkpoints: {output_dir}")
