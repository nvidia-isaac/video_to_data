...
verbose Apache-2.0 boilerplate, or the standard 2-line SPDX header) and
normalizes them to the concise 2-line SPDX header::
...
def normalize(text: str) -> str:
    """Return ``text`` with the standard SPDX header, or unchanged if third-party."""
    ...
    while body and body[0].strip() == "":  # strip existing blank lines after the header
        body.pop(0)

    # Ensure exactly one blank line between the header and the body.
    separator = ["\n"] if body else []
    return "".join(head + HEADER_LINES + separator + body)
