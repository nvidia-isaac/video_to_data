...
- **Retarget image** `robotic-grounding` — IK-retargets hands to the robot, reconstructs
  ...
  2.3.2** container base, pulled from NGC (pinned in `workflow/Dockerfile`).
...
- **NGC access** — the retarget image builds `FROM nvcr.io/nvidia/isaac-lab:2.3.2`, which Docker
  pulls automatically during the build. Make sure `docker login nvcr.io` (user `$oauthtoken`,
  password = your [NGC API key](https://ngc.nvidia.com/signin)) succeeds before you build. No local Isaac Lab install is needed.
...
- `--stages auto` (default) picks the recommended stages for the dataset and prints why.
  For every dataset this ends with an `assess` quality pass (report-only → `{ds}_quality.csv`;
  a failure aborts the run). Override with `--stages load,urdf,processed,support,vis` to drop
  `assess`, or pass any subset.
- `--build` builds the needed image(s) first.
- `--max-sequences N` samples the first N sequences in **filesystem order** — it does **not**
  guarantee any specific sequence. The RL smoke tests in the [README](../README.md#agent-smoke-tests)
  reference `arctic/arctic_processed/dataset_s07_box_grab_01/sharpa_wave`, so to reproduce them pin
  that sequence explicitly instead: `--sequence-pattern s07_box_grab_01` (a regex `re.search`ed over
  the derived `sequence_id`).
...
## 10. Pretrained RL checkpoints (optional: eval without training)

To run eval without training first, download the example checkpoints from Hugging Face
(run `huggingface-cli login` with an authorized token first):

```bash
# on the HOST, from robotic_grounding/
huggingface-cli download nvidia/video-to-data-robotic-grounding-example \
...
for the full eval command and which task ID to use.
