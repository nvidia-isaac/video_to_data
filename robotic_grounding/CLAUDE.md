...
All RL/replay commands assume you are **inside the container** at the package root. Start it with
your dataset root mounted so the `--motion_file` shorthand resolves (it looks under the in-container
`assets/human_motion_data/`): `HUMAN_MOTION_DATA_DIR=<HMD> ./workflow/run.sh start latest 0`. When
issuing from the host, wrap them: `./workflow/run.sh exec latest 0 -- <cmd>`.

**Run the full pipeline on a dataset (from the host):**
```bash
# --sequence-pattern pins the one sequence the smoke tests use (--max-sequences N samples
# the first N in filesystem order and does NOT guarantee dataset_s07_box_grab_01):
python scripts/run_pipeline_docker.py arctic --hmd <HMD> --mano-dir <HMD>/mano --sequence-pattern s07_box_grab_01
...
- **Registry is the single source of truth.** `source/robotic_grounding/robotic_grounding/retarget/dataset_registry.py`
  ...
  a dataset there first — see "Adding a New Dataset" in `workflow/data_pipeline.md`.
...
- **Supported datasets:** taco, hot3d, arctic, grab, h2o, dexycb, oakink2. Adding a new one is a
  separate flow — follow "Adding a New Dataset" in `workflow/data_pipeline.md`.
