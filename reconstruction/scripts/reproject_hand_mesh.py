...
def main():
    ...
    hand_intr   = load_intrinsics(args.hand_intrinsics) if args.hand_intrinsics else None
    ...
    if args.world_results:
        ...
        if args.apply_world_scale and 'world_scale' in wr:
            ...
            verts  *= world_scale
            joints *= world_scale
        elif 'world_scale' in wr and not args.apply_world_scale:
            print(f"world_scale={float(wr['world_scale'].flat[0]):.4f} found but not applied (use --apply_world_scale to enable)")

        if args.smooth_poses_sigma > 0:
            sigma = args.smooth_poses_sigma
            print(f"Smoothing cam_R/cam_t with sigma={sigma} frames...")
            frames_idx = np.arange(n_frames, dtype=float)
