...
Usage:
    python scripts/render_multiview_video.py \\
...
def main():
    ...
    obj_tm_raw = trimesh.load(args.mesh, force='mesh')
    ...
    # --- Load hand mesh ---
    hand_data = np.load(args.hand_mesh, allow_pickle=True)
    ...
    # --- Load pose files ---
    pose_files = sorted(glob.glob(os.path.join(args.poses, '*.json')))
    ...
    if args.frames_folder:
        frame_files = sorted(
            ...
            glob.glob(os.path.join(args.frames_folder, '*.jpg'))
        )
        print(f"RGB underlay: {len(frame_files)} frames from {args.frames_folder}")

    start = args.start
    end   = args.end if args.end is not None else min(n_frames, n_pose_files)

    # --- Fixed-view world frame: frame 0's camera space ---
    # T_cam0_obj: object → frame-0 camera space (CV)
    T_cam0_obj_cv = load_pose_cv(pose_files[start])
    # Object in GL cam0 space (fixed across all frames)
    ...
    print(f"Rendering {end - start} frames at {W}×{H} × 4 panels → {out_w}×{out_h}")
    print(f"Fixed views centred on frame-{start} object pose")
    ...
    # Camera-view: camera sits at GL origin, object world = GL cam space
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for fid in range(start, end):
            ...
            # Object at its raw per-frame pose T_camf_obj (in camf space = fixed world).
            # Fixed cameras are anchored to frame-0; object drifts as camera moves.
            obj_verts_fix_cv = (T_cam_obj_cv[:3, :3] @ obj_verts.T).T + T_cam_obj_cv[:3, 3]
            obj_tm_fix = make_obj_tm(verts_cv_to_gl(obj_verts_fix_cv))

            # Hands: raw camera-space verts (same reference frame as object above)
            hand_fix_list = []
            hand_faces_fix = []
            for h in range(n_hands):
                if fid >= n_frames or vis_mask[h, fid] < 0.5:
