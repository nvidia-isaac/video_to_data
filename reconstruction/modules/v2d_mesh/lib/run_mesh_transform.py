...
def run_mesh_transform(
    ...
    pairs = broadcast_pairs(mesh_paths, transform_paths, mode)

    mesh_cache: dict[str, Mesh] = {}
    for mesh_p, transform_p in pairs:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_mesh", required=True, help="Output file or pattern (e.g. outputs/*.glb or outputs/*/*.glb)")
    parser.add_argument("--mode", choices=["zip", "product"], default="zip")
    args = parser.parse_args()
    run_mesh_transform(args.input_mesh, args.transform, args.output_mesh, mode=args.mode)
