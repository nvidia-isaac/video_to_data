...
from v2d.mv.postprocess.lib.mv_eval_silhouette_mask import mv_eval_silhouette_mask


def mv_eval_silhouette_mask_human_from_config(cfg):
    """Load human mesh data and run 2D silhouette-vs-SAM2 evaluation."""
    ...
    return mv_eval_silhouette_mask(
        cam_names=cam_names,
        ...
        min_bbox_component_pixels=int(
            cfg.get("min_bbox_component_pixels", 3)
        ),
        min_bbox_component_fraction_of_largest=float(
            cfg.get("min_bbox_component_fraction_of_largest", 0.001)
        ),
        render_bbox_padding_pixels=int(
            cfg.get("render_bbox_padding_pixels", 8)
        ),
        render_bbox_padding_fraction=float(
            cfg.get("render_bbox_padding_fraction", 0.1)
        ),
        debug=debug,
        vis_dir=vis_dir,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="2D silhouette-vs-SAM2 evaluation for human mesh"
    )
    ...
    cfg = OmegaConf.load(Path(__file__).parent / "mv_eval_silhouette_mask_human.yaml")
    if args.config_path:
    ...
    mv_eval_silhouette_mask_human_from_config(cfg)
