...
def _summarize_camera_metrics(
    ...
    if frame_indices is None:
        frame_indices = list(range(len(cam_dists)))
    if len(frame_indices) != len(cam_dists):
        raise ValueError(
            f"{cam_name}: frame_indices and chamfer distances differ in length"
        )
    per_frame_mm = [float(v * 1000) for v in cam_dists]
    metrics = {
        "mean_mm": float(arr.mean() * 1000),
        "median_mm": float(np.median(arr) * 1000),
        # Retain the legacy compact list for old readers, but make source-frame
        # identity explicit so skipped frames cannot shift later measurements.
        "per_frame_mm": per_frame_mm,
        "per_frame": [
            {"frame_index": int(frame_idx), "distance_mm": distance_mm}
            for frame_idx, distance_mm in zip(frame_indices, per_frame_mm)
        ],
    }
    if print_summary:
