...
class RigidSilhouetteMaskCameraJob:
    ...
    min_mask_pixels: int
    min_bbox_component_pixels: int
    min_bbox_component_fraction_of_largest: float
    render_bbox_padding_pixels: int
    debug: int
    vis_dir: Path | None
    render_bbox_padding_fraction: float = 0.0
    profile: bool = False
...
class SilhouetteMaskCameraJob:
    ...
    debug: int
    vis_dir: Path | None
    min_bbox_component_pixels: int = 1
    min_bbox_component_fraction_of_largest: float = 0.0
    render_bbox_padding_pixels: int = 0
    render_bbox_padding_fraction: float = 0.0
    profile: bool = False
    show_progress: bool = True
    progress_interval: float = 0.1
...
def filter_mask_components_for_bbox(
    mask: np.ndarray,
    min_component_pixels: int,
    min_component_fraction_of_largest: float = 0.0,
) -> tuple[np.ndarray, dict[str, int | float]]:
    """Remove tiny 8-connected components for bbox construction only.

    The absolute threshold handles isolated one- or two-pixel specks.  The
    relative threshold prevents a handful of pixels far from a large primary
    component from making its bbox flicker, without imposing a large absolute
    minimum on genuinely small objects.
    """
    if min_component_pixels < 1:
        raise ValueError("min_component_pixels must be at least 1")
    if not 0.0 <= min_component_fraction_of_largest <= 1.0:
        raise ValueError("min_component_fraction_of_largest must be in [0, 1]")
    source = mask.astype(bool)
    source_bbox = mask_to_bbox(source)
    if source_bbox is None:
        return np.zeros_like(source), {
            "sam2_bbox_component_count": 0,
            "filtered_sam2_bbox_component_count": 0,
            "removed_sam2_bbox_component_count": 0,
            "removed_sam2_bbox_component_pixels": 0,
            "largest_sam2_bbox_component_pixels": 0,
            "effective_min_sam2_bbox_component_pixels": int(
    ...
    largest_component_pixels = int(areas.max())
    relative_min_pixels = int(np.ceil(
        largest_component_pixels * min_component_fraction_of_largest
    ))
    effective_min_pixels = max(int(min_component_pixels), relative_min_pixels)
    retained = areas >= effective_min_pixels
    keep_labels = np.zeros(component_count, dtype=bool)
    keep_labels[1:] = retained
    filtered = np.zeros_like(source)
    filtered[y0:y1, x0:x1] = keep_labels[labels]
    return filtered, {
        "sam2_bbox_component_count": int(len(areas)),
        "filtered_sam2_bbox_component_count": int(retained.sum()),
        "removed_sam2_bbox_component_count": int((~retained).sum()),
        "removed_sam2_bbox_component_pixels": int(areas[~retained].sum()),
        "largest_sam2_bbox_component_pixels": largest_component_pixels,
        "effective_min_sam2_bbox_component_pixels": effective_min_pixels,
    }
...
def pad_bbox(
    ...
    """Pad a half-open bbox per axis and clip it to ``(width, height)``.

    Fractional padding is measured from the rendered silhouette bbox and is
    applied on each side.  The effective padding is the larger of the fixed
    pixel floor and the fractional amount.
    """
    if padding_pixels < 0:
        raise ValueError("padding_pixels must be nonnegative")
    ...
    x0, y0, x1, y1 = bbox
    pad_x = max(padding_pixels, int(np.ceil((x1 - x0) * padding_fraction)))
    pad_y = max(padding_pixels, int(np.ceil((y1 - y0) * padding_fraction)))
    return (
        max(0, x0 - pad_x),
        max(0, y0 - pad_y),
        min(width, x1 + pad_x),
...
def compute_silhouette_mask_frame_metrics(
    ...
    *,
    ...
    filtered_sam2, component_metrics = filter_mask_components_for_bbox(
        sam2,
        min_bbox_component_pixels,
        min_bbox_component_fraction_of_largest,
    )
    filtered_sam2_bbox = mask_to_bbox(filtered_sam2)
    render_bbox = mask_to_bbox(rendered)
    padded_render_bbox = pad_bbox(
        render_bbox,
        render_bbox_padding_pixels,
        (sam2.shape[1], sam2.shape[0]),
        render_bbox_padding_fraction,
    )
    sam2_bbox_pixels = bbox_area(sam2_bbox)
    filtered_sam2_bbox_pixels = bbox_area(filtered_sam2_bbox)
    render_bbox_pixels = bbox_area(render_bbox)
    padded_render_bbox_pixels = bbox_area(padded_render_bbox)
    bbox_intersection_pixels = bbox_intersection_area(sam2_bbox, render_bbox)
    filtered_padded_bbox_intersection_pixels = bbox_intersection_area(
        filtered_sam2_bbox,
        padded_render_bbox,
    )
    raw_bbox_containment = sam2_bbox_in_render_bbox_ratio(sam2_bbox, render_bbox)
    filtered_padded_bbox_containment = sam2_bbox_in_render_bbox_ratio(
        filtered_sam2_bbox,
        padded_render_bbox,
    )
    if filtered_padded_bbox_containment is None:
        # An otherwise eligible source mask whose components were all filtered
        # is a bbox-containment failure, not an ineligible frame.
        filtered_padded_bbox_containment = 0.0
    bbox_metrics = {
        "sam2_bbox": sam2_bbox,
        "filtered_sam2_bbox": filtered_sam2_bbox,
        "render_bbox": render_bbox,
        "padded_render_bbox": padded_render_bbox,
        "bbox_intersection_pixels": bbox_intersection_pixels,
        "filtered_padded_bbox_intersection_pixels": (
            filtered_padded_bbox_intersection_pixels
        ),
        "sam2_bbox_pixels": sam2_bbox_pixels,
        "filtered_sam2_bbox_pixels": filtered_sam2_bbox_pixels,
        "render_bbox_pixels": render_bbox_pixels,
        "padded_render_bbox_pixels": padded_render_bbox_pixels,
        "sam2_bbox_in_render_bbox_ratio": raw_bbox_containment,
        "filtered_sam2_bbox_in_padded_render_bbox_ratio": (
            filtered_padded_bbox_containment
        ),
        "min_bbox_component_pixels": int(min_bbox_component_pixels),
        "min_bbox_component_fraction_of_largest": float(
            min_bbox_component_fraction_of_largest
        ),
        "render_bbox_padding_pixels": int(render_bbox_padding_pixels),
        "render_bbox_padding_fraction": float(render_bbox_padding_fraction),
        **component_metrics,
    }
    if mask_pixels < min_mask_pixels:
        return {
            ...
            **bbox_metrics,
            ...
            "filtered_sam2_bbox_in_padded_render_bbox_ratio": None,
        }
...
def _summarize_frame_metrics(frame_metrics: list[dict]) -> dict:
        ...
        summary.update(
            {
                ...
                "mean_filtered_sam2_bbox_in_padded_render_bbox_ratio": None,
                "median_filtered_sam2_bbox_in_padded_render_bbox_ratio": None,
                "total_filtered_sam2_bbox_in_padded_render_bbox_ratio": None,
                "total_sam2_bbox_pixels": 0,
                "total_bbox_intersection_pixels": 0,
                "total_filtered_sam2_bbox_pixels": 0,
                "total_filtered_padded_bbox_intersection_pixels": 0,
            }
        )
        return summary
    ...
    filtered_padded_bbox_ratios = np.array(
        [m["filtered_sam2_bbox_in_padded_render_bbox_ratio"] for m in valid],
        dtype=float,
    )
    ...
    total_filtered_sam2_bbox_pixels = sum(
        int(m["filtered_sam2_bbox_pixels"]) for m in valid
    )
    total_filtered_padded_bbox_intersection_pixels = sum(
        int(m["filtered_padded_bbox_intersection_pixels"]) for m in valid
    )
    ...
    summary.update(
        {
            ...
            "mean_filtered_sam2_bbox_in_padded_render_bbox_ratio": float(
                filtered_padded_bbox_ratios.mean()
            ),
            "median_filtered_sam2_bbox_in_padded_render_bbox_ratio": float(
                np.median(filtered_padded_bbox_ratios)
            ),
            "total_filtered_sam2_bbox_in_padded_render_bbox_ratio": float(
                total_filtered_padded_bbox_intersection_pixels
                / total_filtered_sam2_bbox_pixels
                if total_filtered_sam2_bbox_pixels > 0
                else 0.0
            ),
            ...
            "total_filtered_sam2_bbox_pixels": int(
                total_filtered_sam2_bbox_pixels
            ),
            "total_filtered_padded_bbox_intersection_pixels": int(
                total_filtered_padded_bbox_intersection_pixels
            ),
        }
    )
...
def _summarize_object_alignment_camera(
    frame_metrics: list[dict],
    *,
    min_bbox_containment: float,
) -> dict:
    """Summarize filtered/padded SAM2-bbox containment for one camera."""
    if not 0.0 <= min_bbox_containment <= 1.0:
        raise ValueError("min_bbox_containment must be in [0, 1]")

    eligible_frames = 0
    bad_frames = 0
    min_filtered_padded_containment: float | None = None
    min_raw_containment: float | None = None
    max_unexplained_ratio: float | None = None
    longest_bad_run = 0
    bad_spans: list[dict] = []
    run: list[dict] = []

    def _finish_run() -> None:
        nonlocal longest_bad_run, run
        if not run:
            return
        longest_bad_run = max(longest_bad_run, len(run))
        bad_spans.append(
            {
                "start_frame": int(run[0]["frame_idx"]),
                "end_frame": int(run[-1]["frame_idx"]),
                "length": len(run),
                "min_filtered_sam2_bbox_in_padded_render_bbox_ratio": float(
                    min(
                        metric[
                            "filtered_sam2_bbox_in_padded_render_bbox_ratio"
                        ]
                ...
                "max_unexplained_sam2_ratio": float(
                    max(metric["unexplained_sam2_ratio"] for metric in run)
                ),
            }
        )
        run = []
    ...
    for metric in frame_metrics:
        ...
        eligible_frames += 1
        containment = float(
            metric["filtered_sam2_bbox_in_padded_render_bbox_ratio"]
        )
        raw_containment = float(metric["sam2_bbox_in_render_bbox_ratio"])
        unexplained_ratio = float(metric["unexplained_sam2_ratio"])
        min_filtered_padded_containment = (
            containment
            if min_filtered_padded_containment is None
            else min(min_filtered_padded_containment, containment)
        )
        min_raw_containment = (
            raw_containment
            if min_raw_containment is None
            else min(min_raw_containment, raw_containment)
        )
        max_unexplained_ratio = (
            unexplained_ratio
            if max_unexplained_ratio is None
            else max(max_unexplained_ratio, unexplained_ratio)
        )
        if containment < min_bbox_containment:
            bad_frames += 1
            run.append(metric)
        else:
            _finish_run()
    ...
    return {
        ...
        "bad_frame_fraction": float(
            bad_frames / eligible_frames if eligible_frames else 0.0
        ),
        "min_filtered_sam2_bbox_in_padded_render_bbox_ratio": (
            min_filtered_padded_containment
        ),
        "min_sam2_bbox_in_render_bbox_ratio": min_raw_containment,
        "max_unexplained_sam2_ratio": max_unexplained_ratio,
        "longest_bad_run": longest_bad_run,
        "bad_spans": bad_spans,
    }


def _build_object_alignment_quality_gate(
    ...
    *,
    ...
    min_bbox_component_pixels: int,
    render_bbox_padding_pixels: int,
    max_bad_frame_fraction: float,
    min_bbox_component_fraction_of_largest: float = 0.0,
    render_bbox_padding_fraction: float = 0.0,
) -> dict:
    """Build the object-only alignment gate from per-frame camera metrics."""
    if min_segment_pixels < 1:
        raise ValueError("min_segment_pixels must be at least 1")
    if min_bbox_component_pixels < 1:
        raise ValueError("min_bbox_component_pixels must be at least 1")
    if not 0.0 <= min_bbox_component_fraction_of_largest <= 1.0:
        raise ValueError(
            "min_bbox_component_fraction_of_largest must be in [0, 1]"
        )
    if render_bbox_padding_pixels < 0:
        raise ValueError("render_bbox_padding_pixels must be nonnegative")
    if render_bbox_padding_fraction < 0:
        raise ValueError("render_bbox_padding_fraction must be nonnegative")
    if not 0.0 <= max_bad_frame_fraction <= 1.0:
        raise ValueError("max_bad_frame_fraction must be in [0, 1]")

    per_camera = {
        result.cam_name: _summarize_object_alignment_camera(
            result.frame_metrics,
            min_bbox_containment=min_bbox_containment,
        )
        for result in results
    }
    eligible_camera_frames = sum(
        camera["eligible_frames"] for camera in per_camera.values()
    )
    bad_camera_frames = sum(camera["bad_frames"] for camera in per_camera.values())
    failing_cameras = [
        cam_name
        for cam_name, camera in per_camera.items()
        if camera["bad_frame_fraction"] > max_bad_frame_fraction
    ]

    if eligible_camera_frames == 0:
        status = "INCONCLUSIVE"
        reason = f"no frames with sam2_mask_pixels >= {min_segment_pixels}"
    elif failing_cameras:
        status = "FAIL"
        details = ", ".join(
            f"{cam_name} bad frame fraction "
            f"{per_camera[cam_name]['bad_frame_fraction']:.6f} > "
            f"{max_bad_frame_fraction:.6f}"
            for cam_name in failing_cameras
        )
        reason = f"object silhouette bbox containment failed: {details}"
    else:
        status = "PASS"
        reason = ""
    ...
    return {
        ...
        "eligible_camera_frames": eligible_camera_frames,
        "bad_camera_frames": bad_camera_frames,
        "failing_cameras": failing_cameras,
        "per_camera": per_camera,
        "thresholds": {
            "min_object_silhouette_bbox_containment": float(
                min_bbox_containment
            ),
            "min_object_segment_pixels": int(min_segment_pixels),
            "min_object_bbox_component_pixels": int(min_bbox_component_pixels),
            "min_object_bbox_component_fraction_of_largest": float(
                min_bbox_component_fraction_of_largest
            ),
            "object_silhouette_render_bbox_padding_pixels": int(
                render_bbox_padding_pixels
            ),
            "object_silhouette_render_bbox_padding_fraction": float(
                render_bbox_padding_fraction
            ),
            "max_object_silhouette_bad_frame_fraction": float(
                max_bad_frame_fraction
            ),
        },
    }
...
def _debug_canvas(
    ...
    filtered_sam2_mask, _ = filter_mask_components_for_bbox(
        sam2_mask,
        int(metric.get("min_bbox_component_pixels", 1)),
        float(metric.get("min_bbox_component_fraction_of_largest", 0.0)),
    )
    ...
    top = np.concatenate(
        [
            ...
            _panel(
                filtered_sam2_mask,
                (255, 255, 255),
                "Filtered SAM2 bbox (magenta)",
                metric.get("filtered_sam2_bbox"),
    ...
    bottom = np.concatenate(
        [
            _panel(
                render_mask,
                (0, 220, 0),
                "Compare: SAM2 magenta, padded render orange",
                metric.get("padded_render_bbox"),
                bbox_color=(255, 165, 0),
                extra_bbox=metric.get("filtered_sam2_bbox"),
                extra_bbox_color=(255, 0, 255),
            ),
            _panel(
                unexplained,
                (255, 60, 60),
                "Residual: red unexplained, blue over",
                extra_bbox=None,
            ),
        ],
        axis=1,
    )
    over_render_panel = bottom[:, panel_size[0] :, :]
    resized_over_render = cv2.resize(
        over_render.astype(np.uint8),
        panel_size,
        interpolation=cv2.INTER_NEAREST,
    ).astype(bool)
    over_render_panel[resized_over_render] = (80, 160, 255)
    ...
    if metric.get("skipped", False):
        ...
        text = (
            ...
            f"raw_bbox={metric['sam2_bbox_in_render_bbox_ratio']:.3f} "
            "filtered+padded_bbox="
            f"{metric['filtered_sam2_bbox_in_padded_render_bbox_ratio']:.3f} "
            f"noise_px={metric['removed_sam2_bbox_component_pixels']}"
        )
    cv2.putText(
        canvas,
        text,
        ...
        with Renderer(image_size=(W_eval, H_eval)) as renderer:
            ...
            for i in progress:
                ...
                metric = compute_silhouette_mask_frame_metrics(
                    ...
                    min_bbox_component_pixels=job.min_bbox_component_pixels,
                    min_bbox_component_fraction_of_largest=(
                        job.min_bbox_component_fraction_of_largest
                    ),
                    render_bbox_padding_pixels=job.render_bbox_padding_pixels,
                    render_bbox_padding_fraction=job.render_bbox_padding_fraction,
                )
...
def _write_metrics(
    *,
    ...
    tile_shape: tuple[int, int],
    tile_image_size: tuple[int, int] | None,
    object_quality_gate: dict | None = None,
) -> dict:
    ...
    # Per-frame source indices are required by segment-based accuracy QC for
    # both subjects.  Older object reports already contained this field;
    # writing it unconditionally makes the human report equally actionable.
    metrics["frame_metrics"] = {
        result.cam_name: result.frame_metrics for result in results
    }
    if object_quality_gate is not None:
        metrics["quality_gate"] = object_quality_gate
...
def mv_eval_silhouette_mask_rigid_object(
    ...
    profile: bool = False,
    progress_interval: float = 0.1,
    min_bbox_containment: float = 0.8,
    min_bbox_component_pixels: int = 3,
    min_bbox_component_fraction_of_largest: float = 0.001,
    render_bbox_padding_pixels: int = 8,
    render_bbox_padding_fraction: float = 0.1,
    max_bad_frame_fraction: float = 0.05,
) -> dict:
    ...
    jobs = [
        RigidSilhouetteMaskCameraJob(
            ...
            min_bbox_component_pixels=min_bbox_component_pixels,
            min_bbox_component_fraction_of_largest=(
                min_bbox_component_fraction_of_largest
            ),
            render_bbox_padding_pixels=render_bbox_padding_pixels,
            render_bbox_padding_fraction=render_bbox_padding_fraction,
            debug=debug,
            vis_dir=vis_dir,
            profile=profile,
    ...
    quality_gate = _build_object_alignment_quality_gate(
        results,
        min_bbox_containment=min_bbox_containment,
        min_segment_pixels=min_mask_pixels,
        min_bbox_component_pixels=min_bbox_component_pixels,
        min_bbox_component_fraction_of_largest=(
            min_bbox_component_fraction_of_largest
        ),
    ...
    return _write_metrics(
        ...
        tile_shape=tile_shape,
        tile_image_size=tile_image_size,
        object_quality_gate=quality_gate,
    )
