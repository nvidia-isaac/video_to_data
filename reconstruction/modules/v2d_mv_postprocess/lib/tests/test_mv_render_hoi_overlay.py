...
def test_render_hoi_overlay_uses_persistent_object_mesh(monkeypatch, tmp_path):
    ...
    assert all(frame.dtype == np.uint8 for frame in writer.frames)
    assert writer.closed
    assert FakeFrameSource.source.closed


def test_render_hoi_overlay_converts_to_uint8_before_annotation(monkeypatch, tmp_path):
    writer = FakeWriter()
    FakeFrameSource.source = FakeSource()
    FakeRenderer.instances = []
    monkeypatch.setattr(mv_render_hoi_overlay, "FrameSource", FakeFrameSource)
    ...
    put_text = mv_render_hoi_overlay.cv2.putText

    def assert_uint8_image(image, *args, **kwargs):
        assert image.dtype == np.uint8
        return put_text(image, *args, **kwargs)

    monkeypatch.setattr(mv_render_hoi_overlay.cv2, "putText", assert_uint8_image)
