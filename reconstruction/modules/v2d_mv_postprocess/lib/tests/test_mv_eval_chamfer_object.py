...
def test_chamfer_summary_preserves_source_frame_indices():
    summary = mv_eval_chamfer._summarize_camera_metrics(
        "front", [0.010, 0.020], [2, 7]
    )

    assert summary["per_frame"] == [
        {"frame_index": 2, "distance_mm": 10.0},
        {"frame_index": 7, "distance_mm": 20.0},
    ]
    assert summary["per_frame_mm"] == [10.0, 20.0]
