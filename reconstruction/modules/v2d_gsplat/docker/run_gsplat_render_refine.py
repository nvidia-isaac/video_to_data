...
def run_gsplat_render_refine(
    ...
    """
    Two-phase render-and-compare Gaussian object tracking inside Docker.

    Phase 1: selects quality keyframes from FP poses, trains a canonical
             Gaussian with poses fixed (no joint ambiguity).
    Phase 2: freezes the canonical, refines every per-frame pose via
             render-and-compare (masked L1 + SSIM + silhouette BCE).
    ...
    Optional inputs:
      config_path     - YAML config (see RenderRefineConfig for all fields)
      mesh_path       - .obj mesh for Gaussian init
      depth_dir       - depth folder (used only for mesh alignment at init)
      poses_dir       - FP per-frame SE(3) JSONs ({frame:06d}.json, Transform3d)
      person_masks_dir - person silhouette masks (unused in loss, for future use)
    """
    inputs = {
    ...
    run_in_container(
        image=IMAGE_NAME,
        module='v2d.gsplat.lib.run_gsplat_render_refine',
        inputs=inputs,
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Render-and-compare Gaussian object tracking (Docker wrapper)'
    )
