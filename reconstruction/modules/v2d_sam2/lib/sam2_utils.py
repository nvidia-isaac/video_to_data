...
#
# Memory-efficient version of SAM2VideoPredictor that loads frames on-demand
...
def build_sam2_video_predictor_low_mem(
    config_file,
    ckpt_path=None,
    device="cuda",
    mode="eval",
    ...
    **kwargs,
    ...
    hydra_overrides = [
        "++model._target_=sam2_utils.SAM2VideoPredictorLowMem",
    ]
...
def build_sam2_video_predictor_hf(model_id, checkpoint_dir=None, **kwargs):
    config_name, checkpoint_name = HF_MODEL_ID_TO_FILENAMES[model_id]
    if checkpoint_dir and os.path.exists(os.path.join(checkpoint_dir, checkpoint_name)):
        ckpt_path = os.path.join(checkpoint_dir, checkpoint_name)
    else:
        ckpt_path = hf_hub_download(repo_id=model_id, filename=checkpoint_name)
    return build_sam2_video_predictor_low_mem(
...
class LazyFrameLoader:
    """
    A lazy frame loader that loads frames on-demand with LRU caching.
    This significantly reduces memory consumption for long videos.
    """
    ...
    def __init__(
        ...
        cache_size=10,  # Number of frames to keep in memory
    ):
    ...
    def _init_video_file(self):
        """Initialize for video file input."""
        import decord
        decord.bridge.set_bridge("torch")
        
        # Open video reader to get metadata
        vr = decord.VideoReader(self.video_path)
        self.num_frames = len(vr)
        self.video_height, self.video_width, _ = vr[0].shape
        
        # Store video reader (decord supports lazy loading)
    ...
    def _init_image_directory(self):
        ...
        self.img_paths = [os.path.join(jpg_folder, frame_name) for frame_name in frame_names]
        
        # Load first frame to get dimensions
        img_pil = Image.open(self.img_paths[0])
        self.video_width, self.video_height = img_pil.size
        self.loader_type = "images"
    ...
    def _load_frame_from_images(self, frame_idx):
...
class SAM2VideoPredictorLowMem(SAM2Base):
    """
    Memory-efficient version of SAM2VideoPredictor that loads frames on-demand.
    This version uses LazyFrameLoader to avoid loading all frames into memory at once.
    """
    ...
    def __init__(
        ...
        frame_cache_size=10,  # Number of frames to cache in memory
        **kwargs,
    ):
    ...
    def init_state(
        ...
        # Use lazy frame loader instead of loading all frames
        lazy_loader = LazyFrameLoader(
        ...
        inference_state = {}
        inference_state["images"] = lazy_loader  # Store lazy loader instead of tensor
    ...
    @classmethod
    def from_pretrained(cls, model_id: str, **kwargs) -> "SAM2VideoPredictorLowMem":
        """
        ...
        # Convert to low memory version if needed
        if not isinstance(sam_model, cls):
            # Create new instance with same parameters
            low_mem_model = cls(
                fill_hole_area=getattr(sam_model, 'fill_hole_area', 0),
                non_overlap_masks=getattr(sam_model, 'non_overlap_masks', False),
                clear_non_cond_mem_around_input=getattr(sam_model, 'clear_non_cond_mem_around_input', False),
                add_all_frames_to_correct_as_cond=getattr(sam_model, 'add_all_frames_to_correct_as_cond', False),
            )
            # Copy model weights
            low_mem_model.load_state_dict(sam_model.state_dict())
            return low_mem_model
        return sam_model
    ...
    def _get_image_feature(self, inference_state, frame_idx, batch_size):
        ...
        if backbone_out is None:
            ...
            device = inference_state["device"]
            # Use lazy loader to get frame on-demand
            lazy_loader = inference_state["images"]
            frame_tensor = lazy_loader[frame_idx]  # This triggers lazy loading
            # Ensure frame is on the right device and add batch dimension
            if frame_tensor.device != device:
                frame_tensor = frame_tensor.to(device)
            image = frame_tensor.unsqueeze(0)  # Add batch dimension
