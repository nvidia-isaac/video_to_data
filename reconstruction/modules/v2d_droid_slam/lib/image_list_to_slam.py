...
Image folder may contain .png, .jpg, or .jpeg files. Frame index is parsed
from the filename stem as an integer (zero-padded names like 000123.png).
"""
...
def _load_intrinsics_for_frame(
    input_intrinsics_path: str | None,
    input_intrinsics_folder: str | None,
    frame_idx: int,
    fallback: CameraIntrinsics | None,
) -> CameraIntrinsics:
    if input_intrinsics_folder is not None:
        path = os.path.join(input_intrinsics_folder, f"{frame_idx:06d}.json")
        if os.path.exists(path):
            return CameraIntrinsics.load(path)
        if fallback is None:
            raise FileNotFoundError(
                f"No intrinsics file for frame {frame_idx} at {path} and no fallback")
        return fallback
...
def image_list_to_slam(
    ...
    if input_intrinsics_path is not None:
        fallback_intrinsics = CameraIntrinsics.load(input_intrinsics_path)
    elif input_intrinsics_folder is not None:
        # Use the first available file as a fallback for any frame missing its own.
        files = sorted(
        ...
        fallback_intrinsics = CameraIntrinsics.load(files[0])
    ...
    def frame_iter() -> Iterator[Frame]:
        for i, path in enumerate(image_paths):
            ...
            intr = _load_intrinsics_for_frame(
                input_intrinsics_path, input_intrinsics_folder,
                frame_idx, fallback_intrinsics,
            )
            yield Frame(
                frame_idx=frame_idx,
                image_bgr=image_bgr,
                intrinsics=intr,
                prior_depth=_depth_for_frame(input_depth_folder, frame_idx),
                align_to_depth=_depth_for_frame(align_to_depth_folder, frame_idx),
            )
