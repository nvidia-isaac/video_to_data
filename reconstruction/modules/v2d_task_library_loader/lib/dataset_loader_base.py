...
def _object_assets_relative(path: str) -> str:
    """Return `path` relative to (and including) its ``object_assets`` segment.

    A path with no such segment is returned unchanged.
    """
...
class DatasetLoaderBase(ABC):
    ...
    @staticmethod
    def _report_load_outcome(
        matched: int, loaded: int, skipped: list[str], args: Any
    ) -> None:
        """Summarize the load and ABORT (non-zero exit) when it produced nothing.

        A load that skips every sequence it matched must not exit 0. The pipeline
        orchestrator would carry on into the retarget/support/assess stages, which
        read whatever ``{ds}_loaded`` an EARLIER run left behind — reporting a green
        pipeline and a quality CSV over stale artifacts this run never produced.
        """
        num_shards = getattr(args, "num_shards", 1) or 1
        if matched == 0:
            if num_shards > 1:
                # A sharded (OSMO) run may legitimately hand this shard no work.
                print(
                    f"Shard {getattr(args, 'shard_id', 0)} of {num_shards} matched no "
                    "sequences; nothing to load."
                )
                return
            sys.exit(
                "\nNo sequences matched — nothing was loaded and no output was written.\n"
                "  Check the dataset root and the --sequence_id / --sequence_pattern / "
                "--sequence_file filters."
            )

        print(f"Loaded {loaded}/{matched} sequences ({len(skipped)} skipped)")
        if loaded == 0:
            preview = ", ".join(skipped[:5]) + (" ..." if len(skipped) > 5 else "")
            sys.exit(
                f"\nEvery matched sequence was skipped ({len(skipped)}/{matched}); no "
                f"output was written to {getattr(args, 'output_dir', '?')}.\n"
                f"  Skipped: {preview}\n"
                "  See the 'Skipping <id>: <reason>' lines above for the cause — usually "
                "missing/incomplete raw data or object meshes/URDFs for this dataset."
            )
        if skipped:
            print(
                f"  WARNING: {len(skipped)} sequence(s) skipped and NOT written: "
                + ", ".join(skipped[:5])
    ...
    def run(self, args: Any) -> None:
        ...
        if not sequences:
            # Either the filters matched nothing (an error — the caller asked for
            # sequences that don't exist) or this shard is legitimately empty.
            self._report_load_outcome(0, 0, [], args)
            return
        ...
        # Track what this invocation actually produced: a run that skips every
        # sequence must fail rather than leave later stages on stale artifacts.
        loaded = 0
        skipped: list[str] = []
        ...
        for sequence_info in tqdm(sequences):
            try:
                ...
                skipped.append(sequence_info.sequence_id)
                continue
        ...
        self._report_load_outcome(len(sequences), loaded, skipped, args)
