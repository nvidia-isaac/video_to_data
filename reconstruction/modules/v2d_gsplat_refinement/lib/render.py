...
def render_rgb_depth(
    ...
    Returns:
        rgb:      (H, W, 3)
        depth:    (H, W)              -- expected depth (alpha-weighted)
    ...
    if extra_features is not None:
        colors = torch.cat([frame.colors, extra_features], dim=-1)   # (N, 3+F)
        F_extra = extra_features.shape[-1]
    else:
        colors = frame.colors
        F_extra = 0
    ...
    out, alphas, _ = rasterization(
        ...
        far_plane  = far_plane,
        render_mode= "RGB+ED",       # ED = expected depth (alpha-weighted)
    ...
    depth   = out[0, ..., -1]
    alpha   = alphas[0, ..., 0]
    feats   = out[0, ..., 3:3 + F_extra] if F_extra > 0 else None
    return rgb, depth, alpha, feats
