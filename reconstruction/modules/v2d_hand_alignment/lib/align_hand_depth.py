...
Usage:
    python -m v2d.hand_alignment.lib.align_hand_depth \\
    ...
    python -m v2d.hand_alignment.lib.align_hand_depth ... --diag_dir /tmp/hand_depth_diag

    # Per-frame scale instead of global:
    python -m v2d.hand_alignment.lib.align_hand_depth ... --per_frame

    # Per-frame per-hand scale (each hand aligned independently each frame):
    python -m v2d.hand_alignment.lib.align_hand_depth ... --per_hand
"""
...
def align_hand_depth(
    ...
    """Align hand mesh depth to scene depth by estimating a z-scale or z-offset."""
    intr = load_intrinsics(intrinsics_path)
    width  = intr.get('width',  776)
    height = intr.get('height', 1032)

    mesh_intr = load_intrinsics(mesh_intrinsics_path) if mesh_intrinsics_path else intr
    fx, fy = mesh_intr['fx'], mesh_intr['fy']
    cx, cy = mesh_intr['cx'], mesh_intr['cy']
    if mesh_intrinsics_path:
    ...
    for fid in range(min(n_frames, len(depth_files))):
        ...
        for h in range(n_hands):
            ...
            if valid:
                ...
                if diag_dir and diag_frames and fid in diag_frames:
                    diag_scale = (1.0 + val / zb[np.isfinite(zb)].mean()) if use_offset else val
                    save_diag(fid, None, zb, moge_depth, 1.0, diag_scale, diag_dir)
    ...
    # ------------------------------------------------------------------
    if smooth_sigma > 0 and per_hand:
        print(f"\nSmoothing per-frame per-hand offsets with sigma={smooth_sigma} frames...")
        for h in range(n_hands):
