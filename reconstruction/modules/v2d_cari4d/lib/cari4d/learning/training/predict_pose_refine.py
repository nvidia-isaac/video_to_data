...
from learning.foundationpose_data import FoundationPoseBatchData, FoundationPoseRefineDataProcessor
from learning.models.refine_network import RefineNet
from Utils import *


@torch.inference_mode()
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, xyz_map, normal_map=None, mesh_diameter=None, cfg=None, glctx=None, mesh_tensors=None, dataset:FoundationPoseRefineDataProcessor=None):
  H,W = depth.shape[:2]
  args = []
  method = 'box_3d'
  ...
  pose_data = FoundationPoseBatchData(rgbAs=rgbAs, rgbBs=rgbBs, depthAs=None, depthBs=None, normalAs=normalAs, normalBs=normalBs, poseA=poseA, poseB=None, xyz_mapAs=xyz_mapAs, xyz_mapBs=xyz_mapBs, tf_to_crops=tf_to_crops, Ks=Ks, mesh_diameters=mesh_diameters)
  pose_data = dataset.transform_batch(batch=pose_data, H_ori=H, W_ori=W, bound=1) # RGB is normalized here
...
class PoseRefinePredictor:
  def __init__(self, cfg=None):
    ...
    ckpt_file = self.load_config(cfg)
    ...
    self.dataset = FoundationPoseRefineDataProcessor(self.cfg)
    self.init_model(ckpt_file)
  ...
  @torch.inference_mode()
  def predict(self, rgb, depth, K, ob_in_cams, xyz_map, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, iteration=5, rgb_only=False):
    '''
    @rgb: np array (H,W,3)
    ...
    for _ in range(iteration): # what is the meaning of using different iterations?? it then uses the scorenet to select the best one
      # B_in_cams is reused here, so it starts with coarse prediction, and then refine it.
      ...
      for b in range(0, pose_data.rgbAs.shape[0], bs):
        ...
        with torch.cuda.amp.autocast(enabled=self.amp):
          output = self.model(A,B) # A is the noisy render, B is the input image
        for k in output:
