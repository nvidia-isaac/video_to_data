...
from learning.foundationpose_data import FoundationPoseBatchData, FoundationPoseScoreDataProcessor
from learning.models.score_network import *
from Utils import *


def vis_batch_data_scores(pose_data, ids, scores, pad_margin=5, no_text=False, vis_size=100):
  ...
  for id in ids:
    ...
    s = vis_size / row.shape[0] # resize to 200
    row = cv2.resize(row, fx=s, fy=s, dsize=None)
    if not no_text:
      txt = f'{scores[id]:.3f}' if not isinstance(scores[id], str) else scores[id]
      row = cv_draw_text(row, text=f'id:{id}, score:{txt}', uv_top_left=(10,10), color=(0,255,255), fontScale=0.5, thickness=2)
    canvas.append(row)
...
@torch.no_grad()
def make_crop_data_batch(render_size, ob_in_cams, mesh, rgb, depth, K, crop_ratio, normal_map=None, mesh_diameter=None, glctx=None, mesh_tensors=None, dataset:FoundationPoseScoreDataProcessor=None, cfg=None):
  H,W = depth.shape[:2]
  ...
  pose_data = FoundationPoseBatchData(rgbAs=rgbAs, rgbBs=rgbBs, depthAs=depthAs, depthBs=depthBs, normalAs=normalAs, normalBs=normalBs, poseA=poseAs, xyz_mapAs=xyz_mapAs, tf_to_crops=tf_to_crops, Ks=Ks, mesh_diameters=mesh_diameters)
  pose_data = dataset.transform_batch(pose_data, H_ori=H, W_ori=W, bound=1) # rgb is normalized here
...
class ScorePredictor:
  def __init__(self, amp=True):
    ...
    code_dir = os.path.dirname(os.path.realpath(__file__))
    weights_dir = os.environ.get('FOUNDATIONPOSE_WEIGHTS_DIR', f'{code_dir}/../../weights')
    ...
    self.dataset = FoundationPoseScoreDataProcessor(self.cfg)
  ...
  def predict(self, rgb, depth, K, ob_in_cams, normal_map=None, get_vis=False, mesh=None, mesh_tensors=None, glctx=None, mesh_diameter=None, num_vis=None, no_text=False, rgb_only=False, return_pose_data=False):
    '''
    @rgb: np array (H,W,3)
    '''
    ...
    def find_best_among_pairs(pose_data:FoundationPoseBatchData):
      ids = []
      scores = []
    ...
    if get_vis:
      ...
      visualization_ids = ids if num_vis is None else ids[:num_vis]
      canvas = vis_batch_data_scores(pose_data, ids=visualization_ids, scores=scores, no_text=no_text)
      return (scores, canvas, pose_data) if return_pose_data else (scores, canvas)

    return (scores, None, pose_data) if return_pose_data else (scores, None)
