...
class MHRHead(nn.Module):
    ...
    def mhr_forward(
        ...
        # Add singleton batch dimension if needed (e.g. single-person output from process_one_image)
        if len(global_trans.shape) == 1:
            global_trans = global_trans[None]
        if len(global_rot.shape) == 1:
            global_rot = global_rot[None]
        if len(body_pose_params.shape) == 1:
            body_pose_params = body_pose_params[None]
        if hand_pose_params is not None and len(hand_pose_params.shape) == 1:
            hand_pose_params = hand_pose_params[None]

        batch_size = global_trans.shape[0]
        if expr_params is None:
            expr_params = torch.zeros(
                batch_size, self.num_face_comps,
                device=global_trans.device, dtype=global_trans.dtype,
            )
        elif len(expr_params.shape) == 1:
