...
_GRAVITY_ALIGNMENT_METHOD_Z_UP = "geocalib_opencv_up_to_down_zup_frame"
_GRAVITY_ALIGNMENT_METHOD_VECTOR = "geocalib_opencv_up_to_down_vector_align"
...
def _geocalib_gravity_to_opencv_camera(gravity_camera: np.ndarray) -> np.ndarray:
    """Convert GeoCalib's vertical vector to result-bundle gravity/down.

    GeoCalib's zero roll/pitch vector is image-up ``[0, -1, 0]`` in the same
    OpenCV camera axes used by this bundle (+Y down, +Z forward). The result
    bundle stores physical gravity/down, so the whole vector is negated. A
    Y-only flip gets level cameras right but leaves pitched-camera Z inverted.
    """
    return _normalize(-np.asarray(gravity_camera, dtype=np.float64))
...
def gravity_align_result_bundle(
    ...
    *,
    ...
    GeoCalib stores the image vertical/up direction in camera coordinates. The
    result bundle uses physical gravity/down in OpenCV camera axes (+X right,
    +Y image-down, +Z forward), so the full GeoCalib vector is negated before
    combining it with camera-to-world poses.
    ...
    manifest["gravity_alignment"] = {
        ...
        "geocalib_up_vector_negated_to_gravity": True,
