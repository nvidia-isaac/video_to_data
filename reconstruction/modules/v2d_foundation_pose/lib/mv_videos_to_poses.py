...
from .backends import DEFAULT_BACKEND, SUPPORTED_BACKENDS
from .multiview_tracker import MultiViewTracker, RegistrationFailure
from .symmetry import (
    DEFAULT_CONTINUOUS_SYMMETRY_STEP_DEG,
    identity_symmetry_spec,
    load_symmetry_spec,
)


POSE_TRACKING_METADATA_NAME = "pose_tracking_metadata.json"
...
def _tracking_policy(
    ...
    *,
    ...
    return {
        "recovery_enabled": bool(recovery_config.enabled),
        "repair_enabled": bool(repair_config.enabled),
        "smooth_across_recovery": bool(smooth_across_recovery),
    }


def _tracking_artifacts(repair_enabled: bool) -> dict[str, str]:
    artifacts = {
        "poses": "poses.npy",
        "pose_valid_mask": "pose_valid_mask.npy",
        "tracking_status": "tracking_status.json",
        "runtime": "foundation_pose_runtime.json",
        "forward_poses": "poses_forward.npy",
        "forward_pose_valid_mask": "pose_valid_mask_forward.npy",
        "forward_tracking_status": "tracking_status_forward.json",
    }
    if repair_enabled:
        artifacts.update(
            {
                "repaired_poses": "poses_repaired.npy",
                "repair_candidate_poses": "poses_repair_candidate.npy",
                "repair_status": "repair_status.json",
            }
        )
    return artifacts


def _failed_pose_tracking_metadata(
    *,
    frame_start: int,
    cam_names: list[str],
    error: Exception,
    ...
    return {
        ...
        "source_frame_end_exclusive": int(frame_start),
        "pose_count": 0,
        "camera_names": list(cam_names),
        "error": str(error),
        "pose_valid_count": 0,
        "camera_supported_frame_count": 0,
        "tracking_policy": _tracking_policy(
            recovery_config,
            repair_config,
            smooth_across_recovery=smooth_across_recovery,
        ),
        "tracking_status_counts": {"registration_failed": 1},
        "artifacts": {"runtime": "foundation_pose_runtime.json"},
    }


def _build_pose_tracking_metadata(
    *,
    frame_start: int,
    ...
    cam_names: list[str],
    registration_camera_names: list[str],
    highest_visibility_registration_camera: str,
    registration_visible_ratios: dict[str, float],
    recovery_config: RecoveryConfig,
    repair_config: RepairConfig,
    smooth_across_recovery: bool,
) -> dict[str, object]:
    pose_count = int(len(filtered_poses))
    if frame_end_exclusive - frame_start != pose_count:
        raise RuntimeError(
            "pose count does not match the source frame interval "
            f"([{frame_start}, {frame_end_exclusive}), poses={pose_count})"
        )
    select_mask = np.asarray(select_mask, dtype=bool)
    if select_mask.shape != (pose_count, len(cam_names)):
        raise RuntimeError(
            "camera selection mask does not match poses and cameras "
            f"({select_mask.shape} != ({pose_count}, {len(cam_names)}))"
        )
    pose_valid_mask = np.asarray(pose_valid_mask, dtype=bool)
    if pose_valid_mask.shape != (pose_count,):
        raise RuntimeError(
            "pose validity mask does not match poses "
            f"({pose_valid_mask.shape} != ({pose_count},))"
        )
    if len(tracking_status) != pose_count:
        raise RuntimeError(
            "tracking status count does not match poses "
            f"({len(tracking_status)} != {pose_count})"
        )

    camera_frame_counts = select_mask.astype(np.int64).sum(axis=0)
    status_counts = Counter(
        str(status.get("status", "unknown")) for status in tracking_status
    )
    return {
        "schema_version": 1,
        "status": "completed",
        "source_frame_start": int(frame_start),
        "source_frame_end_exclusive": int(frame_end_exclusive),
        "pose_count": pose_count,
        "camera_names": list(cam_names),
        "registration_camera_names": list(registration_camera_names),
        "highest_visibility_registration_camera": (
            highest_visibility_registration_camera
        ),
        "registration_visible_ratios": dict(registration_visible_ratios),
        "tracking_camera_frame_counts": {
            name: int(camera_frame_counts[index])
            for index, name in enumerate(cam_names)
        },
        "pose_valid_count": int(pose_valid_mask.sum()),
        "camera_supported_frame_count": int(select_mask.any(axis=1).sum()),
        "tracking_policy": _tracking_policy(
            recovery_config,
            repair_config,
            smooth_across_recovery=smooth_across_recovery,
        ),
        "tracking_status_counts": dict(sorted(status_counts.items())),
        "artifacts": _tracking_artifacts(repair_config.enabled),
    }
...
def mv_videos_to_poses(
    ...
    Args:
        ...
        backend: FoundationPose inference backend.
        ...
        symmetry_path: optional path to a BOP-style symmetry annotation JSON
            ...
            the file exists, per-view registrations at the slice's first frame are
            canonicalized into a common equivalence-class representative
            before averaging. If None or missing, behavior matches the
            no-symmetry case.
        continuous_symmetry_step_deg: sampling interval for a finite continuous
            axial symmetry. Unrestricted rotation is detected without sampling.
        ...
        frame_end_exclusive: optional exclusive frame limit for processing a
            bounded sequence slice. None processes through the source end.
        clamp_frame_end_exclusive: when true, clamp an end index beyond the
            available source length; otherwise an out-of-range end is an error.
        frame_start: inclusive source-frame index at which registration starts.
    """
    pose_path = Path(pose_path)
    pose_path.parent.mkdir(parents=True, exist_ok=True)
    metadata_path = pose_path.parent / POSE_TRACKING_METADATA_NAME
    ...
    symmetry_spec = identity_symmetry_spec(continuous_symmetry_step_deg)
    symmetry_group = None
    if symmetry_path is not None and Path(symmetry_path).exists():
        symmetry_spec = load_symmetry_spec(
            symmetry_path, continuous_step_deg=continuous_symmetry_step_deg,
        )
        symmetry_group = list(symmetry_spec.transforms)
        print(
            f"Loaded symmetry mode={symmetry_spec.mode}: "
            f"{symmetry_spec.candidate_count} candidates from {symmetry_path}"
        )
    else:
    ...
    source_frame_count = frame_sources[0].n_frames
    for j, (fs, ds, ms) in enumerate(zip(frame_sources, depth_sources, mask_sources)):
        if (
            fs.n_frames != source_frame_count
            or ds.n_frames != source_frame_count
            or ms.n_frames != source_frame_count
        ):
            raise ValueError(
                f"camera {cam_names[j]}: frame count mismatch "
                f"(rgb={fs.n_frames}, depth={ds.n_frames}, "
                f"mask={ms.n_frames}, expected={source_frame_count})"
            )
    ...
    if frame_end_exclusive is None:
        effective_frame_end = source_frame_count
    else:
        if type(frame_end_exclusive) is not int or frame_end_exclusive <= 0:
            raise ValueError("frame_end_exclusive must be a positive integer")
        if frame_end_exclusive > source_frame_count:
            if clamp_frame_end_exclusive:
                effective_frame_end = source_frame_count
            else:
                raise ValueError(
                    "frame_end_exclusive must be inside the available frame range "
                    f"(requested={frame_end_exclusive}, available={source_frame_count})"
                )
        else:
            effective_frame_end = frame_end_exclusive
    if frame_start >= effective_frame_end:
        raise ValueError(
            "FoundationPose frame slice must contain at least one frame "
            f"(range=[{frame_start}, {effective_frame_end}))"
        )
    num_frames = effective_frame_end - frame_start
    register_debug_path = (
        str(
            pose_path.parent
            / f"register_debug_frame_{frame_start:06d}.json"
        )
        if debug >= 1
        else None
    )
    tracker = MultiViewTracker(
        ...
        full_rotational_symmetry=symmetry_spec.full_rotational,
    ...
    runtime_info = dict(tracker.runtime_info)
    runtime_info["symmetry"] = symmetry_spec.provenance()
    with open(pose_path.parent / "foundation_pose_runtime.json", "w") as stream:
        json.dump(runtime_info, stream, indent=2, sort_keys=True)
        stream.write("\n")
    all_poses = []
    select_mask = []
    output_poses = []
    ...
    print(
        "Starting multi-view tracking for source frames "
        f"[{frame_start}, {effective_frame_end}) across {num_cameras} cameras"
    )
    all_sources = frame_sources + depth_sources + mask_sources
    source_iterators = [source.iter_frames() for source in all_sources]
    for source_iterator in source_iterators:
        for _ in range(frame_start):
            next(source_iterator)
    frame_iterators = source_iterators[:num_cameras]
    depth_iterators = source_iterators[num_cameras : 2 * num_cameras]
    mask_iterators = source_iterators[2 * num_cameras :]
    registration_camera_names: list[str]
    highest_visibility_registration_camera: str
    registration_visible_ratios: dict[str, float]
    tracking_frame_end_exclusive = effective_frame_end

    for local_index in tqdm(
        range(num_frames),
        desc=f"Tracking from source frame {frame_start}",
    ):
        source_frame = frame_start + local_index
        rgbs = [next(iterator) for iterator in frame_iterators]
        depths = [
            DepthImage.from_array(next(iterator)).depth
            for iterator in depth_iterators
        ]
        masks = [next(iterator) > 128 for iterator in mask_iterators]
        ...
        for j in range(num_cameras):
            ...
            if depths[j].shape[:2] != rgbs[j].shape[:2]:
                depths[j] = cv2.resize(
                    depths[j],
                    (rgbs[j].shape[1], rgbs[j].shape[0]),
                    interpolation=cv2.INTER_LINEAR,
                )
            if masks[j].shape[:2] != rgbs[j].shape[:2]:
                masks[j] = cv2.resize(
                    masks[j].astype(np.uint8),
                    (rgbs[j].shape[1], rgbs[j].shape[0]),
        ...
        try:
            ...
            if local_index == 0:
                failure = _failed_pose_tracking_metadata(
        ...
        status = _status_with_camera_names(
            result.status,
            cam_names,
            frame_index=local_index,
        )
        status["source_frame_index"] = int(source_frame)
        front_depth_stats = _masked_depth_stats(
            depths[front_cam_idx], masks[front_cam_idx]
        )
        status["front_camera_name"] = cam_names[front_cam_idx]
        status["front_masked_depth_mean_m"] = front_depth_stats["mean_m"]
        status["front_masked_depth_valid_pixels"] = front_depth_stats[
            "valid_pixels"
        ]
        pose_valid = bool(status.get("pose_valid", bool(select_idx.any())))

        if not recovery_config.enabled and not pose_valid:
            if local_index == 0:
                error = RegistrationFailure(
                    "registration produced no valid target pose"
                )
                failure = _failed_pose_tracking_metadata(
                    frame_start=frame_start,
                    cam_names=cam_names,
                ...
                raise error
            tracking_frame_end_exclusive = source_frame
            tqdm.write(
                "FoundationPose tracking lost all camera overlap at "
                f"source frame {source_frame}; stopping the pose stream"
            )
            break

        if local_index == 0:
            registration_camera_indices = np.flatnonzero(select_idx)
            if len(registration_camera_indices) == 0:
                error = RegistrationFailure(
                    "registration produced no supported camera"
                )
                failure = _failed_pose_tracking_metadata(
                    frame_start=frame_start,
                    cam_names=cam_names,
                ...
                raise error
            highest_visibility_camera_index = int(
                registration_camera_indices[
                    np.argmax(visible_ratios[registration_camera_indices])
                ]
            )
            registration_camera_names = [
                cam_names[index] for index in registration_camera_indices
            ]
            highest_visibility_registration_camera = cam_names[
                highest_visibility_camera_index
            ]
            registration_visible_ratios = {
                name: float(visible_ratios[index])
                for index, name in enumerate(cam_names)
            }
        ...
        if debug >= 2 and local_index % 30 == 0:
            for j in range(num_cameras):
                vis = rgbs[j].copy()
                if pose_valid:
                ...
                iio.imwrite(
                    debug_image_dirs[j] / f"{source_frame:06d}.png",
                    vis,
                )
    ...
    metadata = _build_pose_tracking_metadata(
        frame_start=frame_start,
        ...
        registration_visible_ratios=registration_visible_ratios,
        recovery_config=recovery_config,
        repair_config=repair_config,
        smooth_across_recovery=smooth_across_recovery,
    )
    ...
    print(f"Saved pose tracking metadata to {metadata_path}")
    ...
    if debug >= 1:
        ...
        _render_tiled_debug_video(
            ...
            select_mask, pose_valid_mask, pose_path, len(filtered_poses),
            frame_start=frame_start, scale=scale,
        )

    # if debug > 1:
    #     _render_debug_videos(
    #         filtered_poses, cam_names, frame_sources, Ks_orig, Ts, tracker,
    #         pose_path, len(filtered_poses), frame_start=frame_start,
    #     )
    return metadata
            ...
            for frame_idx in range(anchor_idx - 1, start_floor - 1, -1):
                try:
                    rgbs, depths, masks = _load_tracking_inputs_at(
                        ...
                        frame_index=(
                            frame_context.source_frame_start + frame_idx
                        ),
...
def mv_videos_to_poses_from_config(cfg):
    ...
    mv_videos_to_poses(
        ...
        continuous_symmetry_step_deg=cfg.get(
            "continuous_symmetry_step_deg", DEFAULT_CONTINUOUS_SYMMETRY_STEP_DEG,
        ),
        scale=scale,
        ...
        frame_end_exclusive=cfg.get("frame_end_exclusive", None),
        clamp_frame_end_exclusive=cfg.get("clamp_frame_end_exclusive", False),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--continuous_symmetry_step_deg", type=float, default=None,
        help="Sampling interval for finite continuous symmetry (default: 10 degrees)",
    )
    ...
    for key in [
        ...
        "mask_dir", "mesh_path", "symmetry_path", "continuous_symmetry_step_deg",
        "weights_dir", "output_dir", "backend",
        "scale", "debug", "frame_start", "frame_end_exclusive",
        "clamp_frame_end_exclusive",
    ]:
