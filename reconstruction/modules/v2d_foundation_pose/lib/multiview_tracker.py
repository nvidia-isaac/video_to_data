...
class RegistrationFailure(RuntimeError):
    """Raised when no camera produces a usable target-mesh registration."""
...
def _canonicalize_poses_with_indices(
    ...
    candidates = pose_array[:, None, :, :] @ transforms[None, :, :, :]
    ref_rotation = np.asarray(reference, dtype=float).reshape(4, 4)[:3, :3]
    relative = np.einsum(
        "ij,nkjl->nkil", ref_rotation.T, candidates[:, :, :3, :3], optimize=True,
    )
    cosines = np.clip(
        (np.trace(relative, axis1=2, axis2=3) - 1.0) / 2.0, -1.0, 1.0,
    )
    best_indices = np.argmax(cosines, axis=1)
    rows = np.arange(len(pose_array))
    canonicals = candidates[rows, best_indices]
    distances = np.arccos(cosines[rows, best_indices])
    return list(canonicals), [
        (int(index), float(distance))
        for index, distance in zip(best_indices, distances)
    ]
...
def _dump_register_debug(
    ...
    record = {
        ...
        "avg_pose": (
            np.asarray(avg_pose).tolist() if avg_pose is not None else None
        ),
    }
    with open(p, "w") as f:
...
class MultiViewTracker:
    ...
    def __init__(
        ...
        with _suppress_fp_logging():
            ...
            scorer = predictor_backend.scorer
            refiner = predictor_backend.refiner
        ...
        self.full_rotational_symmetry = bool(full_rotational_symmetry)
        ...
        if self.full_rotational_symmetry:
            print(
                "[MultiViewTracker] unrestricted rotational symmetry enabled; "
                "holding the first reliable fused rotation"
            )
        elif symmetry_group is None:
    ...
    def register(
        ...
        """Register one initialization frame across all cameras.
        ...
        if not select_idx.any():
            if debug_dump_path is not None:
                _dump_register_debug(
                    ...
                    canonical_world_poses=raw_world_poses,
                    snap_indices_per_iter=[],
                    avg_pose=None,
                )
            ratios = ", ".join(f"{value:.6f}" for value in visible_ratios)
            raise RegistrationFailure(
                "target-mesh registration did not produce sufficient mask "
                "overlap in any camera "
                f"(cutoff={self.visible_ratio_cutoff_low:.6f}, "
                f"visible_ratios=[{ratios}])"
            )
    ...
    def _track_registered_frame(
        ...
        if (
            self.full_rotational_symmetry
            and int(select_idx.sum()) > 0
            and int(select_idx.sum()) < self.recovery_min_views
        ):
            # For an unrestricted rotational object, orientation cannot be
            # recovered from another view.  Re-registering every non-selected
            # camera is therefore both unobservable and needlessly expensive.
            # Preserve the reference rotation and continue fusing the tracked
            # translation from the views that do contain the object.
            world_poses = self._poses_with_held_rotation(world_poses)
            avg_pose = self._weighted_se3_mean_from_poses(
                world_poses, Ts, visible_ratios, select_idx,
            )
            avg_pose = self._pose_with_rotation(
                avg_pose, self.pose_last[:3, :3],
            )
            self._sync_to_avg(avg_pose, Ts)
            self._lost_frame_count = 0
            self._under_supported_frame_count = 0
            self.last_status = self._make_status(
                ...
                recovery_mode="full_rotational_observable_translation",
            )
            return avg_pose, world_poses, visible_ratios, select_idx
        ...
        if self.full_rotational_symmetry:
            avg_pose = self._pose_with_rotation(avg_pose, self.pose_last[:3, :3])
        self._sync_to_avg(avg_pose, Ts)
        self._lost_frame_count = 0
        self._under_supported_frame_count = 0
        self.last_status = self._make_status(
    ...
    def _fuse_registered_poses(
        ...
        if self.full_rotational_symmetry:
            ...
            reference_rotation = (
                np.asarray(self.pose_last, dtype=float)[:3, :3]
                if self.pose_last is not None
                else np.asarray(raw_world_poses[ref_idx], dtype=float)[:3, :3]
            )
            world_poses = [
                self._pose_with_rotation(pose, reference_rotation)
                for pose in raw_world_poses
            ]
            avg_pose = self._weighted_se3_mean_from_poses(
                world_poses, Ts, visible_ratios, select_idx,
            )
            avg_pose = self._pose_with_rotation(avg_pose, reference_rotation)
            snap_indices_per_iter.append([(0, 0.0) for _ in raw_world_poses])
        elif self.symmetry_group is not None and len(self.symmetry_group) > 1:
            scores_for_ref = visible_ratios.copy()
            scores_for_ref[~select_idx] = -np.inf
            ref_idx = int(np.argmax(scores_for_ref))
    ...
    def _handle_under_supported_tracking(
        ...
        tracked_world_poses = self._poses_with_held_rotation(tracked_world_poses)
        avg_pose = self._weighted_se3_mean_from_poses(
    ...
    def _poses_with_held_rotation(
        self, world_poses: list[np.ndarray],
    ) -> list[np.ndarray]:
        if not self.full_rotational_symmetry or self.pose_last is None:
            return list(world_poses)
        rotation = np.asarray(self.pose_last, dtype=float)[:3, :3]
        return [self._pose_with_rotation(pose, rotation) for pose in world_poses]
    ...
    def _avg_poses(
        ...
        avg_pose = self._weighted_se3_mean_from_poses(
            self._poses_with_held_rotation(world_poses), Ts, visible_ratios, select_idx,
        )
        if self.full_rotational_symmetry:
            avg_pose = self._pose_with_rotation(avg_pose, self.pose_last[:3, :3])
