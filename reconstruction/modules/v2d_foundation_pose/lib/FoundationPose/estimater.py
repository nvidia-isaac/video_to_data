...
class FoundationPose:
  ...
  def make_rotation_grid(self, min_n_views=40, inplane_step=60):
    ...
    # Ensure proper data type and memory layout for C++ module
    rot_grid = np.ascontiguousarray(rot_grid, dtype=np.float32)
    symmetry_tfs = np.ascontiguousarray(self.symmetry_tfs.data.cpu().numpy(), dtype=np.float32)
    
    rot_grid = mycpp.cluster_poses(30, 99999, rot_grid, symmetry_tfs)
    rot_grid = np.asarray(rot_grid)
