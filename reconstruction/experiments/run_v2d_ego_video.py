...
def _multiview_render(scaled_mesh, poses_smooth_dir, hand_mesh_smooth,
    ...
    """Run multiview render, printing a fallback command if EGL fails."""
    result = subprocess.run([
        sys.executable, "scripts/render_multiview_video.py",
        "--mesh",          scaled_mesh,
        "--poses",         poses_smooth_dir,
        "--hand_mesh",     hand_mesh_smooth,
        "--intrinsics",    intrinsics_stable,
        "--frames_folder", frames_dir,
        "--output",        multiview_mp4,
        "--fps",           "25",
    ])
    if result.returncode != 0:
        print(f"  [WARN] Multiview render failed (EGL context issue). Run directly:")
        print(f"    python scripts/render_multiview_video.py \\\n"
              f"      --mesh {scaled_mesh} --poses {poses_smooth_dir} \\\n"
              f"      --hand_mesh {hand_mesh_smooth} --intrinsics {intrinsics_stable} \\\n"
              f"      --frames_folder {frames_dir} --output {multiview_mp4} --fps 25")
...
if not os.path.exists(moge_hand_aligned):
    ...
    subprocess.run([
        ...
        "--hand_intrinsics",   hand_intrinsics_path,
        "--target_intrinsics", moge_intrinsics_stable,
        "--output",            moge_hand_aligned,
    ], check=True)
else:
...
if not os.path.exists(moge_hand_perhand):
    ...
    subprocess.run([
        ...
        "--input",      moge_hand_aligned,
        "--depth",      moge_depth_dir,
        "--intrinsics", moge_intrinsics_stable,
        "--output",     moge_hand_perhand,
        "--per_hand", "--align", "offset",
    ], check=True)
else:
...
if not os.path.exists(moge_hand_smooth):
    ...
    subprocess.run([
        ...
        "--input",  moge_hand_perhand,
        "--output", moge_hand_smooth,
...
if not os.path.exists(da3_hand_perhand):
    ...
    subprocess.run([
        sys.executable, "scripts/align_hand_depth.py",
        "--input",      da3_hand_aligned,
        "--depth",      da3_depth_dir,
        "--intrinsics", da3_intrinsics_stable,
        "--output",     da3_hand_perhand,
        "--per_hand", "--align", "offset",
    ], check=True)
else:
...
if not os.path.exists(da3_hand_smooth):
    ...
    subprocess.run([
        sys.executable, "scripts/smooth_hand_mesh.py",
        "--input",  da3_hand_perhand,
        "--output", da3_hand_smooth,
        "--sigma",  str(SMOOTH_SIGMA),
    ], check=True)
else:
