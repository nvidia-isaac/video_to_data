...
def run_gsplat(intrinsics_path: str, dev: bool = False, quick: bool = False, frame_step: int = 1):
    ...
    # 'cosine'      — smooth annealing from 1.0 to lr_decay_final (recommended).
    # 'exponential' — faster early decay, same final value.
    # 'none'        — constant LR (disables decay).
    lr_decay_schedule   = 'cosine'
    lr_decay_final      = 0.1   # final LR as fraction of initial (0.1 = 10× decay)
    ...
    # Frame sampling strategy during optimization.
    ...
    # Hard-negative mining params (only used when frame_sampling='hard_negative').
    # beta=0 disables (uniform random); beta=1 linear; beta=2 quadratic focus.
    # eps: uniform floor so no frame is ever completely skipped.
    hard_mining_beta    = 2.0
    hard_mining_eps     = 0.1
