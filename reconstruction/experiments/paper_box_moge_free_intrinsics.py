"""
Pipeline: paper_box with MoGe depth (no input intrinsics) + GT intrinsics for FP

Tests whether the issue with MoGe tracking is caused by using GT intrinsics as
input to MoGe, vs MoGe estimating its own focal length freely.

MoGe runs unconstrained, but GT intrinsics are passed to FoundationPose and
rendering (same as the GT depth runs), so intrinsics are not a confound.

Prerequisite: run paper_box_pipeline.py first (needs frames/ and masks/ to exist).

Run from reconstruction/:
    python experiments/paper_box_moge_free_intrinsics.py
"""
...
# GT intrinsics used for FP + rendering (not passed to MoGe)
...
# MoGe outputs (free intrinsics)
depth_dir          = f"{OUTPUT_DIR}/depth_moge_free"
intrinsics_dir     = f"{OUTPUT_DIR}/intrinsics_moge_free"
intrinsics_stable  = f"{OUTPUT_DIR}/intrinsics_moge_free_stable.json"

# FP outputs
poses_dir          = f"{OUTPUT_DIR}/poses_moge_free"
poses_smoothed_dir = f"{OUTPUT_DIR}/poses_moge_free_smoothed"
renders_dir        = f"{OUTPUT_DIR}/renders_moge_free"
renders_smooth_dir = f"{OUTPUT_DIR}/renders_moge_free_smoothed"
...
# ---------------------------------------------------------------------------
# Step 4: MoGe depth — no input intrinsics
# ---------------------------------------------------------------------------
if not _has_files(depth_dir):
    print("Step 4: MoGe depth estimation (free intrinsics)...")
    run_video_to_depth(
    ...
    print("Step 4: Skipping (depth_moge_free already computed)")

# ---------------------------------------------------------------------------
# Step 5: Stabilise MoGe intrinsics (informational — not used for FP)
# ---------------------------------------------------------------------------
if not os.path.exists(intrinsics_stable):
    print("Step 5: Stabilising MoGe intrinsics (for reference)...")
...
# ---------------------------------------------------------------------------
# Step 6: FoundationPose — MoGe depth, GT intrinsics
# ---------------------------------------------------------------------------
if not _has_files(poses_dir):
    print("Step 6: FoundationPose tracking (MoGe depth, GT intrinsics)...")
    run_video_to_poses(
        video_path=VIDEO_PATH,
        depth_folder=depth_dir,
    ...
    print("Step 6: Skipping (poses_moge_free already computed)")
...
renders_mp4        = f"{OUTPUT_DIR}/renders_moge_free.mp4"
renders_smooth_mp4 = f"{OUTPUT_DIR}/renders_moge_free_smoothed.mp4"
...
stitch_videos(
    ...
    f"{OUTPUT_DIR}/comparison_moge_free.mp4",
)

print(f"Done. Output: {OUTPUT_DIR}/comparison_moge_free.mp4")
