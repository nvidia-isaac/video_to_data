"""
Pipeline: airplane Session_20260310_130642_f50
  DA3 metric depth -> depth alignment -> FoundationPose (mask_depth=True) -> EKF -> render

Steps:
  1. Align reference frame (frame 0) DA3 metric depth to object mesh via FP-guided grid search
     -> depth_da3metric_aligned/depth_reference.png
  2. Align all DA3 metric frames to reference depth via ICP + affine
     -> depth_da3metric_aligned/
  3. FP tracking with aligned depth + depth masking enabled
  4. EKF smoothing
  5. Render raw + smoothed poses
  6. Comparison video

Run from reconstruction/:
    python experiments/airplane_da3metric_aligned_depth_fp.py
"""
...
# ---------------------------------------------------------------------------
# Check for frame 0 output (depth_reference.png is written by step 1, so check for frame file)
...
if not os.path.exists(comparison_mp4):
    print("Step 7: Creating comparison video (aligned depth | raw FP | smoothed FP)...")
