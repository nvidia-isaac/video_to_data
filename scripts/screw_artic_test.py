"""Test the screw_assembly ARTICULATION: thread_test fixed base + SDF screw on a revolute joint.
Script the SDF screwdriver to spin tip-in-slot and read the screw_spin joint -> does it follow?"""
import argparse
import numpy as np
from isaaclab.app import AppLauncher
parser = argparse.ArgumentParser()
parser.add_argument("--omega", type=float, default=2.0)
parser.add_argument("--tip_in", type=float, default=0.004)
...
ART = f"{ASSETS}/screw_assembly/screw_assembly.usd"
...
HEAD = np.array([0.0769, -0.0003, 0.5986])
